...
def calculate_half_life(series: pl.Series) -> Optional[float]:
    """
    ...
    # Lag the series by one period
    lagged_series = series.shift(1).fill_null(0)
    delta_series = series - lagged_series
    
    # Drop the first row as it will have a zero lag
    delta_series = delta_series.slice(1)
    lagged_series = lagged_series.slice(1)

    # Regress the change in the series on the lagged series
    x = lagged_series.to_numpy().reshape(-1, 1)
    y = delta_series.to_numpy()
    ...
    # The coefficient for the lagged series
    if len(results.params) < 2:
        return None
        
    lambda_ = results.params[1]
...
def calculate_zero_crossings(series: pl.Series) -> int:
    """
    Counts the number of times a series crosses its mean.
    """
    if series.len() < 2:
        return 0
    
    # Center the series around its mean
    centered_series = series - series.mean()
    
    # Determine the sign of each element
    signs = np.sign(centered_series.to_numpy())
    
    # Find where the sign changes
    sign_changes = ((np.roll(signs, 1) - signs) != 0).astype(int)
    
    # The first element is a boundary condition, not a crossing
    sign_changes[0] = 0
